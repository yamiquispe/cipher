...
export const validateInput = (offset, message, operation) => {
  ...
  if (offset === '' || offset.indexOf('.') === 0 ||
    offset.indexOf(',') === 1) {
    messageModal.text = 'Por favor, ingrese un número válido.';
  } else if (offset.indexOf('.') === 1 || offset.indexOf(',') > 1) {
    // Casos: 9.90 (al ingresar 9.90); 9,90 (al ingresar 9.9)
    messageModal.text = 'El desplazamiento corresponde a un número decimal.';
  } else if (message === '') {
    ...
    messageModal.text = 'El mensaje ingresado contiene un carácter inválido.';
  } else {
